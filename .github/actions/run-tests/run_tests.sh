  ...
  docker run -d --name $container_name \
    ...
    -c "
      ...
      if [ -n \"\${TEST_WHEELHOUSE_PACKAGES:-}\" ]; then
        ...
        case \" \${TEST_WHEELHOUSE_PACKAGES} \" in
          *\" ovrtx \"*)
            ./isaaclab.sh -p -c \"import hashlib,importlib.metadata,json,os,pathlib; from packaging.version import Version; manifest=json.loads(pathlib.Path(os.environ['TEST_WHEELHOUSE_MANIFEST']).read_text(encoding='utf-8')); record=manifest.get('ovrtx') or manifest.get('packages', {}).get('ovrtx', {}); wheel=pathlib.Path(os.environ['TEST_WHEELHOUSE_PATH']) / record.get('file', ''); expected=record.get('version') or manifest.get('ovrtx_version'); actual=importlib.metadata.version('ovrtx'); digest=hashlib.file_digest(wheel.open('rb'), 'sha256').hexdigest(); import ovrtx; from ovrtx import _version; expected_ref=record.get('source_ref'); expected_sha=record.get('source_sha'); print(f'Resolved ovrtx package version: {actual}'); print(f'Wheelhouse manifest ovrtx version: {expected}'); print(f'Wheelhouse ovrtx SHA-256: {digest}'); print(f'Imported ovrtx provenance: {_version.gitbranch}@{_version.githash}'); raise SystemExit(0 if Version(actual) == Version(expected) and digest == record.get('sha256') and _version.gitbranch == expected_ref and _version.githash == expected_sha else f'ovrtx wheelhouse mismatch: installed {actual}, manifest {expected}, sha {digest}, provenance {_version.gitbranch}@{_version.githash}')\"
            ;;
        esac
        case \" \${TEST_WHEELHOUSE_PACKAGES} \" in
          *\" ovstage \"*)
            ./isaaclab.sh -p -c \"import hashlib,importlib.metadata,json,os,pathlib; from packaging.version import Version; manifest=json.loads(pathlib.Path(os.environ['TEST_WHEELHOUSE_MANIFEST']).read_text(encoding='utf-8')); record=manifest.get('ovstage') or manifest.get('packages', {}).get('ovstage', {}); wheel=pathlib.Path(os.environ['TEST_WHEELHOUSE_PATH']) / record.get('file', ''); expected=record.get('version') or manifest.get('ovstage_version'); actual=importlib.metadata.version('ovstage'); digest=hashlib.file_digest(wheel.open('rb'), 'sha256').hexdigest(); import ovstage; runtime=getattr(ovstage, '__version__', actual); print(f'Resolved ovstage package version: {actual}'); print(f'Wheelhouse manifest ovstage version: {expected}'); print(f'Wheelhouse ovstage SHA-256: {digest}'); raise SystemExit(0 if Version(actual) == Version(expected) and Version(runtime) == Version(expected) and digest == record.get('sha256') else f'ovstage wheelhouse mismatch: installed {actual}, import {runtime}, manifest {expected}, sha {digest}')\"
            ;;
        esac
      fi
